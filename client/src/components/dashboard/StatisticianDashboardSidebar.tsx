...
export default function StatisticianDashboardSidebar({
  activeTab,
  onTabChange,
  mobileOpen,
  setMobileOpen,
}: StatisticianDashboardSidebarProps) {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { user } = useAppSelector(selectAuthState);
  const [logoutPending, setLogoutPending] = useState(false);
  const activeMeta = getStatisticianTabMeta(activeTab);
    ...
    <>
      ...
      >
        <div className="relative flex h-full min-h-0 flex-col overflow-hidden bg-[linear-gradient(135deg,#000054_82%,#1a1a6e_18%,#000054_100%)] px-5 py-4 text-white shadow-[0_32px_90px_-42px_rgba(0,0,84,0.95)]">
          <div className="pointer-events-none absolute inset-x-4 top-0 h-40 rounded-full bg-primary/10 blur-3xl" />
          ...
          <div className="relative flex items-start justify-between">
            ...
            >
              ...
              <div>
                ...
                <p className="text-[10px] text-white/65">Statistician</p>
              </div>
            </button>
          ...
          <nav className="relative mt-4 flex-1 min-h-0 overflow-y-auto pr-1">
            <div className="space-y-1">
              {statisticianNavigation.map((item) => {
          ...
          <div className="relative mt-3 rounded-2xl border border-white/12 bg-white/8 p-3">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-xl bg-primary/20 text-[10px] font-semibold text-primary">
                {user?.photoURL ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={user.photoURL}
                    alt={user.displayName || user.name || "Statistician"}
                    className="h-full w-full object-cover"
                  />
                ) : (
                  (user?.displayName || user?.name || user?.email || "S")
                    .slice(0, 1)
                    .toUpperCase()
                )}
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-xs font-semibold">
                  {user?.displayName || user?.name || "Statistician"}
                </p>
            ...
            <button
              ...
              className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-3 py-2 text-xs font-semibold text-white transition hover:bg-primary-hover disabled:cursor-not-allowed disabled:opacity-70"
            >
              <MdLogout size={16} />
