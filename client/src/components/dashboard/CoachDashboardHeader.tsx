...
const fallbackRoutes = {
  attendance_alert: "/dashboard/coach#training",
  performance_alert: "/dashboard/coach#performance",
  match_alert: "/dashboard/coach#matches",
  system_alert: "/dashboard/coach#notifications",
  system: "/dashboard/coach#notifications",
};
...
export default function CoachDashboardHeader({
  ...
  const userName =
    user?.displayName || user?.name || user?.email?.split("@")[0] || "Coach";
          ...
          <div className="min-w-0">
            ...
            <div className="mt-2 flex min-w-0 flex-col">
              ...
              <p className="truncate text-sm text-slate-500">
                {currentDateLabel} · Ready for selection, {userName}
              </p>
            </div>
          </div>
        </div>
